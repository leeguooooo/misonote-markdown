...
export default function EditButton({ docPath, className = '' }: EditButtonProps) {
  ...
  if (!isAuthenticated) {
    return (
      <Link
        href="/admin"
        className={`inline-flex items-center gap-2 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg transition-colors ${className}`}
        title="需要管理员权限"
      >
